...
import business.qzmj.c2s.iclass.SQZMJ_SendGift;
...
public class CQZMJSendGift extends PlayerHandler {
	...
	public void handle(Player player, WebSocketRequest request, String message)
    	...
    	if (null == baseRoom){
    		request.error(ErrorCode.NotAllow, "CQZMJSendGift not find room:"+roomID);
    		return;
    	}

		AbsRoomPos roomPos = baseRoom.getRoomPosMgr().getPosByPosID(req.pos);
    	if (null == roomPos) {
    		request.error(ErrorCode.NotAllow, "CQZMJSendGift not find pos player in this room pos:"+req.pos);
    		return;
		}
    	...
    	if (null == data){
    		request.error(ErrorCode.NotAllow, "CQZMJSendGift not find productId:"+req.productId);
    		return;
    	}

    	AbsRoomPos sendRoomPos = baseRoom.getRoomPosMgr().getPosByPid(player.getPid());
    	if(null == sendRoomPos){
    		request.error(ErrorCode.NotAllow, "CQZMJSendGift not find pid player in this room pid:"+player.getPid());
    		return;
    	}

        if(data.Num > 0) {
	        if(!player.getFeature(PlayerCurrency.class).checkAndConsumeItemFlow(PrizeType.valueOf(data.Type), data.Num,  ItemFlow.SendGift)){
	            request.error(ErrorCode.NotAllow, "CQZMJSendGift your money not enought  type:"+ PrizeType.valueOf(data.Type) +",num "+data.Num);
	            return;
	        }
        }
        request.response();
    	baseRoom.getRoomPosMgr().notify2All(SQZMJ_SendGift.make(req.roomID, sendRoomPos.getPosID(), req.pos, req.productId));
	}
