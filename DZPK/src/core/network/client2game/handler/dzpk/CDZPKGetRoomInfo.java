 ...
 */
public class CDZPKGetRoomInfo extends PlayerHandler {
    ...
    public void handle(Player player, WebSocketRequest request, String message) throws IOException {
        ...
        // 检查房间是否存在	
        if (null == room) {
            request.error(ErrorCode.NotFind_Room, "CDZPKGetRoomInfo null == room");
            return;
        }
        AbsRoomPos roomPos = room.getRoomPosMgr().getPosByPid(player.getPid());
        // 检查玩家是否在房间中	
        if (null == roomPos) {
            request.error(ErrorCode.NotFind_Player, "CDZPKGetRoomInfo null == roomPos");
            return;
        }
        // 获取房间信息	
