...
public class CPDKAddDouble extends PlayerHandler{
	...
	public void handle(Player player, WebSocketRequest request, String message) throws WSException, IOException {
    	...
    	if (null == room){
    		request.error(ErrorCode.NotAllow, "CPDKAddDouble not find room:"+clientPack.roomID);
    		return;
    	}
    	PDKRoomSet set =  (PDKRoomSet) room.getCurSet();
    	if(null == set){
    		request.error(ErrorCode.NotAllow, "CPDKAddDouble not set room:"+clientPack.roomID);
    		return;
    	}
    	set.onAddDouble(request,clientPack);		
	}
}
