 ...
 */
public class CPDKRoomInvitationOperation extends PlayerHandler {
    ...
    public void handle(Player player, WebSocketRequest request, String message) throws IOException {
        ...
        if (Objects.isNull(room)) {
            request.error(ErrorCode.NotFind_Room, "CPDKRoomInvitationOperation NotFind_Room");
            return;
        }
        AbsRoomPos roomPos = room.getRoomPosMgr().getPosByPid(player.getPid());
        if (Objects.isNull(roomPos)) {
            request.error(ErrorCode.NotFind_Pos, "CPDKRoomInvitationOperation NotFind_Pos");
            return;
        }
        request.response();
