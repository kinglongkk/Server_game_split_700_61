...
//        if(clazz.getSimpleName().equals("BaseCreateRoom")){
//            System.out.println("BaseCreateRoom");
//        }
        T entity = null;
        try {
...
//                    field.set(entity, value);
                    if (val != null) {
                        ...
                        field.set(entity, Enum.valueOf(fieldClazz, (String) val));
                    }
                    continue;
                }
                Document doc = (Document) val;
                if (!doc.isEmpty()) {
                    field.set(entity, toBean(doc, fieldClazz));
                }
//                document.put(key, toBean((Document) val, fieldClazz));
            }
        } catch (Exception e) {
            CommLogD.error("toBean() error , clazz:" + clazz.getName(), e);
        }
     ...
     * @return
     */
    public static <T> List<Document> toListDocument(List<T> entityList) {
...
//                  }
                ...
                //枚举
                if (Enum.class.isAssignableFrom(fieldClazz)) {
                    document.put(key, value == null ? null : value.toString());
                    continue;
                }
                document.put(key, toDocument(value));
            } catch (Exception e) {
                CommLogD.error("toDocument() , error clazz=" + entity.getClass().getName(), e);
            }
        }
        return document;
    }
...
//      }
    ...
    private static void toList(Object value, List list, Class itemClazz) throws InstantiationException, IllegalAccessException {
        ...
        if (value.getClass() == ArrayList.class) {
            if(!isBaseClass(itemClazz)) {
                List<Document> documents = (ArrayList) value;
    ...
    @SuppressWarnings("rawtypes")
    private static boolean isBaseClass(Class itemClazz) {

        //此时为基本类型
        if (itemClazz == Integer.class) {
            return true;
        } else if (itemClazz == Long.class) {
            return true;
        } else if (itemClazz == String.class) {
            return true;
        } else if (itemClazz == Double.class) {
            return true;
        } else if (itemClazz == Float.class) {
            return true;
        } else if (itemClazz == Boolean.class) {
            return true;
        } else {
//            throw new RuntimeException("nonsupport type !!!");
            return false;
