 ...
 *
 */
public class CNNGoldRoom extends PlayerHandler {
	...
	public void handle(Player player, WebSocketRequest request, String message) throws IOException {
		...
		if (data == null) {
			request.error(ErrorCode.NotAllow, "CNNGoldRoom do not find practiceId");
			return;
		}
		// 游戏配置
		CNN_CreateRoom createClientPack = new CNN_CreateRoom();
		// 公共房间配置
		BaseRoomConfigure<CNN_CreateRoom> configure = new BaseRoomConfigure<CNN_CreateRoom>(PrizeType.Gold,
				NNAPP.GameType(), createClientPack.clone(), new RobotRoomConfig(data.getBaseMark(),data.getMin(),data.getMax(),clientPack.getPracticeId()));
