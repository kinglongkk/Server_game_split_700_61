...
public class NNRoomSet_LZNN extends NNRoomSet {


    public NNRoomSet_LZNN(NNRoom room) {
        super(room);
    }
...
//		this.sendTuZhuPoint();
    ...
    public NN_GameStatus getStartStatus() {
        // TODO 自动生成的方法存根
        return NN_GameStatus.NN_GAME_STATUS_BET;
    }
    ...
    public void setDefeault() {
        ...
        if (this.room.getCurSetID() == 1) {
            backerPos = roomPos.getPosID();
        } else {
            backerPos = (roomPos.getPosID() + this.room.getCurSetID() - 1) % this.getPlayingCount();
        }
        this.setBackerPos(backerPos, false);
    }
}
