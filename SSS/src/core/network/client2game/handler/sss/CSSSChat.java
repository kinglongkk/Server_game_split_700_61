...
public class CSSSChat extends PlayerHandler {
    ...
    public void handle(Player player, WebSocketRequest request, String message) throws IOException, WSException {
        ...
        if (room == null) {
            request.error(ErrorCode.NotAllow, "CSSSChat room you in room:" + req.targetID);
            return;
        }
        if (room.checkGaoJiXuanXiang(GaoJiTypeEnum.FORBIDDEN_WORDS)) {
            request.error(ErrorCode.NotAllow, "CCTWSKChat room you in CCTWSKChat:" + req.targetID);
            return;
        }
