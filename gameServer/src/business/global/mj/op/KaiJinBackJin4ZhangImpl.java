 ...
 * @author leo_wi
 * <p>
 * 开金 随机抽取一张,没有放回去，减去1则为金 万，条，筒，中发白，东西南北一个轮回
 *      中发白算一起，如翻开白则发是金；
 *  	东南西北算一起，如翻开东则北是金；
 *  	翻开的那张牌不能抓；
 *  	例如第一张牌是二万，那么一万就是宝。
 ...
 *
 */
public class KaiJinBackJin4ZhangImpl implements OpCard {
    ...
    public void kaiJinCard (AbsMJSetPos mSetPos) {
        ...
        if (card.getType() < MJSpecialEnum.NOT_HUA.value()) {
            // 添加金牌
            if (mSetPos.getSet().getmJinCardInfo().addJinCard(backJinJin(card))) {
                // 金牌添加成功
                return;
            } else {
                // 添加金失败，重新开金
