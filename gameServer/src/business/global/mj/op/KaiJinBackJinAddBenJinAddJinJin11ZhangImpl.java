 ...
 * @author leo_wi
 * <p>
 * 开金 随机抽取一张,没有放回去，减去1+本身+加上1 则为金 万，条，筒，中发白，东西南北一个轮回
 */
public class KaiJinBackJinAddBenJinAddJinJin11ZhangImpl implements OpCard {
    ...
    public void opKaiJin(AbsMJSetPos mSetPos) {
        ...
        if (mSetPos.getSet().getmJinCardInfo().sizeJin() >= mSetPos.getSet().getmJinCardInfo().getKaiJinNum()) {
            MJCard benJin = mSetPos.getSet().getmJinCardInfo().getJin(1);
            // 开金通知。
            mSetPos.getSet().kaiJinNotify(benJin, jinJin(benJin));
            return;
        }
        // 开金
    ...
    public void kaiJinCard(AbsMJSetPos mSetPos) {
        ...
        if (godInfo.isGodCardMode() && godInfo.getJin(0) > 0) {
            ...
            card = mSetPos.getSet().getSetCard().getGodCard(jin);
        } else {
            card = mSetPos.getSet().getMJSetCard().pop(false);
        }
        if (null == card) {
            // 没有牌
            return;
        ...
        if (card.getType() < MJSpecialEnum.NOT_HUA.value()) {
            // 添加金牌
            if (mSetPos.getSet().getmJinCardInfo().addJinCard(backJinJin(card)) && mSetPos.getSet().getmJinCardInfo().addJinCard(card) && mSetPos.getSet().getmJinCardInfo().addJinCard(jinJin(card))) {
                // 金牌添加成功
                return;
            } else {
                // 添加金失败，重新开金
