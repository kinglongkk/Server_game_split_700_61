 ...
 * @author leo_wi
 * <p>
 * 每局开局发牌时同时翻开一张牌，该牌的顺一位和二位做为混牌；
 */
public class KaiJinBenShenAddJin1AddJin2_11ZhangImpl implements OpCard {
    ...
    public void opKaiJin(AbsMJSetPos mSetPos) {
        ...
        if (mSetPos.getSet().getmJinCardInfo().sizeJin() >= mSetPos.getSet().getmJinCardInfo().getKaiJinNum()) {
            MJCard jinJin1 = mSetPos.getSet().getmJinCardInfo().getJin(1);
            MJCard jinJin2 = mSetPos.getSet().getmJinCardInfo().getJin(2);
            // 开金通知。
            mSetPos.getSet().kaiJinNotify(jinJin1, jinJin2);
            return;
        }
        // 开金
    ...
    public void kaiJinCard(AbsMJSetPos mSetPos) {
        ...
        if (godInfo.isGodCardMode() && godInfo.getJin(0) > 0) {
            type = godInfo.getJin(0);
        }
        // 摸牌开金
        card = mSetPos.getSet().getMJSetCard().pop(false, type);
        if (null == card) {
            // 没有牌
            return;
        }
        ...
        if (card.getType() < MJSpecialEnum.NOT_HUA.value()) {
            // 添加金牌
            MJCard jinJin1 = jinJin(card);
            MJCard jinJin2 = jinJin(jinJin1);
            if (mSetPos.getSet().getmJinCardInfo().addJinCard(card) && mSetPos.getSet().getmJinCardInfo().addJinCard(jinJin1) && mSetPos.getSet().getmJinCardInfo().addJinCard(jinJin2)) {
                // 金牌添加成功
                return;
            } else {
                // 添加金失败，重新开金
