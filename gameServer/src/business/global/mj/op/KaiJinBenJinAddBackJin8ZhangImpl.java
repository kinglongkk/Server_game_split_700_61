 ...
 * @author leo_wi
 * <p>
 * 开金 随机抽取一张,然后放回去，减去1+本身 则为金 万，条，筒，中发白，东西南北一个轮回
 * 中发白算一起，如翻开白则发是金；
 * 	东南西北算一起，如翻开东则北是金；
 * 	翻开的那张牌不能抓；
 * 	例如第一张牌是二万，那么一万就是宝。
 * 	此时牌里就剩下4张一万，4张二万；
 */
public class KaiJinBenJinAddBackJin8ZhangImpl implements OpCard {
    ...
    public void kaiJinCard(AbsMJSetPos mSetPos) {
        ...
        if (godInfo.isGodCardMode() && godInfo.getJin(0) > 0) {
            ...
            card = new MJCard(jin > 1000 ? jin : jin * 100 + 1);
        } else {
        ...
        if (card.getType() < MJSpecialEnum.NOT_HUA.value()) {
            // 添加金牌
            if (mSetPos.getSet().getmJinCardInfo().addJinCard(card) && mSetPos.getSet().getmJinCardInfo().addJinCard(jinJin(card))) {
                // 金牌添加成功
                return;
            } else {
                // 添加金失败，重新开金
