 ...
 */
public class KaiJinJin1AddJin2_8ZhangImpl implements OpCard {
    ...
    public void kaiJinCard(AbsMJSetPos mSetPos) {
        ...
        if (card.getType() < MJSpecialEnum.NOT_HUA.value()) {
            ...
            if (mSetPos.getSet().getmJinCardInfo().addJinCard(jinJin1) && mSetPos.getSet().getmJinCardInfo().addJinCard(jinJin2)) {
                ((MJTemplateRoomSet) mSetPos.getSet()).setBenJin(card.cardID);
                // 金牌添加成功
                return;
            } else {
                // 添加金失败，重新开金
