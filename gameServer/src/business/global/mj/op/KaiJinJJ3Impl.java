...
/**
 * 开金
 * 	发完牌后，系统随机翻开一张牌，翻开的牌加1和减1即为混；
 * @author Huaxing
 *
 */
public class KaiJinJJ3Impl implements OpCard {
	...
	public void kaiJinCard (AbsMJSetPos mSetPos) {
		...
		if (card.getType() < MJSpecialEnum.NOT_HUA.value()) {
			// 添加金牌
			if (mSetPos.getSet().getmJinCardInfo().addJinCard(backJinJin(card)) && mSetPos.getSet().getmJinCardInfo().addJinCard(jinJin(card))) {
				// 金牌添加成功
				return;
			} else {
				// 添加金失败，重新开金
