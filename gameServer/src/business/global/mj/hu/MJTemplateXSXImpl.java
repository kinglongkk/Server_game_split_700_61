 ...
 */
public class MJTemplateXSXImpl extends BaseHuCard {
    /**
     * @param mSetPos
     * @param mCardInit
     * @param <T>
    ...
    public <T> Object checkHuCardReturn(AbsMJSetPos mSetPos, MJCardInit mCardInit) {
        ...
        if (checkSiXi(mSetPos, mCardInit, false)) {
            return OpPointEnum.XiaoSiXi;
        }
        return OpPointEnum.Not;
    }
     ...
     * @return
     */
    public boolean checkSiXi(AbsMJSetPos mSetPos, MJCardInit mCardInit, boolean daSiXi) {
        int sizeJin = 0;
        long count;
        //找到中发白
        ...
        for (int i = 41; i <= 44; i++) {
            ...
            if (count < 3) {
                ...
                if (!mSetPos.publicCardTypeList().stream().anyMatch(k -> k / 100 == finalI)) {
                    //公共牌没有找金替代
                    sizeJin = Math.toIntExact(!daSiXi ? 3 : 4 - count);
