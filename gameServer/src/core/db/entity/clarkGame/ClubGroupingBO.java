 ...
 * 亲友圈分组
 * */
@TableName(value = "ClubGrouping")
@Data
@NoArgsConstructor
public class ClubGroupingBO extends BaseEntity<ClubGroupingBO> {
	...
	@DataBaseField(type = "varchar(255)", fieldname = "grouping", comment = "分组")
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `ClubGrouping` ("
                + "`id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '自增主键',"
                + "`clubID` bigint(20) NOT NULL DEFAULT '0' COMMENT '俱乐部ID',"
                + "`createTime` int(11) NOT NULL DEFAULT '0' COMMENT '创建时间',"
                + "`grouping` varchar(255) NOT NULL DEFAULT ''  COMMENT '分组',"
                + "PRIMARY KEY (`id`) ,"
                + "KEY `clubID` (`clubID`) USING BTREE"
                + ") COMMENT='亲友圈分组'  DEFAULT CHARSET=utf8 AUTO_INCREMENT=" + (ServerConfig.getInitialID() + 1);
        return sql;
    }
