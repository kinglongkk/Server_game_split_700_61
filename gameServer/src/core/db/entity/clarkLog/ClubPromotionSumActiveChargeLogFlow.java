...
@TableName(value = "ClubPromotionSumActiveChargeLog" )
@Data
@Deprecated
public class ClubPromotionSumActiveChargeLogFlow extends BaseClarkLogEntity<ClubPromotionSumActiveChargeLogFlow> {
    ...
    public ClubPromotionSumActiveChargeLogFlow() {
    }


    public ClubPromotionSumActiveChargeLogFlow(long pid, long clubId, long unionId, int reason, double num, double curRemainder, double preValue, int type, long partnerPid) {
        this.pid = pid;
        this.clubId = clubId;
    ...
    public String getInsertSql() {
        return "INSERT INTO ClubPromotionSumActiveChargeLog"
                + "(`server_id`, `timestamp`, `date_time`, `pid`, `clubId`, `unionId`, `reason`, `num`, `curRemainder`, `preValue`, `type`, `partnerPid`)"
                + "values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    }

    public static String getCreateTableSQL() {
        String sql = "CREATE TABLE IF NOT EXISTS `ClubPromotionSumActiveChargeLog` ("
                ...
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='亲友圈推广员总活跃值消费日志' DEFAULT CHARSET=utf8";
        return sql;
    }
