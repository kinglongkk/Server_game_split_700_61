...
@TableName(value = "ClubRoomCardChargeLog" )
@Data
public class ClubRoomCardChargeLogFlow extends BaseClarkLogEntity<ClubRoomCardChargeLogFlow> {
    ...
    private long clubID;
    @DataBaseField(type = "int(11)", fieldname = "status", comment = "操作类型 0为默认状态,1为正常，2禁用3解散，4启用，5修改，6服务器重启，7后台 ,8游戏回退,9玩家转入转出,10俱乐部关闭,11记录消耗错误先回退后消耗")
    ...
    public ClubRoomCardChargeLogFlow() {
    }


    public ClubRoomCardChargeLogFlow(long pid,long clubID, int status,int reason, int num, int cur_remainder, int pre_value, int type,int gameType,int rechargeType,long agentsID,int level,int cityId) {
        this.pid = pid;
        this.status = status;
        this.reason = reason;
        this.num = num;
    ...
    public String getInsertSql() {
        return "INSERT INTO ClubRoomCardChargeLog"
                + "(`server_id`, `timestamp`, `date_time`, `pid`, `clubID`, `status`, `reason`, `num`, `cur_remainder`, `pre_value`, `type`,`gameType`,`rechargeType`,`agentsID`,`level`,`cityId`)"
                + "values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?,?)";
    }

    public static String getCreateTableSQL() {
        String sql = "CREATE TABLE IF NOT EXISTS `ClubRoomCardChargeLog` ("
                ...
                + "`pid` bigint(20) NOT NULL DEFAULT '0' COMMENT '玩家pid 当status为9玩家转入转出,10俱乐部关闭 才有赋值 此值参考 ',"
                + "`clubID` bigint(20) NOT NULL DEFAULT '0' COMMENT '俱乐部ID',"
                + "`status` int(11) NOT NULL DEFAULT '0' COMMENT '操作类型 0为默认状态,1为正常，2禁用3解散，4启用，5修改，6服务器重启，7后台 ,8游戏回退,9玩家转入转出,10俱乐部关闭,11记录消耗错误先回退后消耗',"
                ...
                + "`type` int(11) NOT NULL DEFAULT '0' COMMENT '类型(2:消耗,3回退 9俱乐部转出到玩家 10玩家房卡转入俱乐部 )',"
                + "`gameType` int(11) NOT NULL DEFAULT '-1' COMMENT '游戏类型',"
                + "`rechargeType` int(11) NOT NULL DEFAULT '0' COMMENT '类型(8:俱乐部)',"
