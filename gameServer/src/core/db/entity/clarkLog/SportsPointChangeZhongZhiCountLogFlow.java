...
@TableName(value = "SportsPointChangeZhongZhiCountLog" )
@Data
public class SportsPointChangeZhongZhiCountLogFlow extends BaseClarkLogEntity<SportsPointChangeZhongZhiCountLogFlow> {
    ...
    private long roomId;
    public SportsPointChangeZhongZhiCountLogFlow() {
    }

    public SportsPointChangeZhongZhiCountLogFlow(long pid, long clubId, long unionId, int reason, double num, double curRemainder, double preValue, int type, int gameId, int cityId, long roomId) {
        this.pid = pid;
        this.clubId = clubId;
    ...
    public String getInsertSql() {
        return "INSERT INTO SportsPointChangeZhongZhiCountLog"
                + "(`serverId`, `timestamp`, `dateTime`, `pid`, `clubId`,`unionId`, `reason`, `num`, `curRemainder`, `preValue`, `type`,`gameId`,`cityId`, `roomId`)"
                + "values(?, ?, ?, ?, ?, ?,?, ?, ?, ?, ?, ?,?,?)";
    }

    public static String getCreateTableSQL() {
        String sql = "CREATE TABLE IF NOT EXISTS `SportsPointChangeZhongZhiCountLog`("
                ...
                + ") COMMENT='中至竞技点变换日志统计' DEFAULT CHARSET=utf8";
        return sql;
    }
