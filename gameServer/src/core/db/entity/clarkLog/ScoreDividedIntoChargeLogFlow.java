 ...
 * 积分收益分成日志
 */
@TableName(value = "ScoreDividedIntoChargeLog")
@Data
@NoArgsConstructor
public class ScoreDividedIntoChargeLogFlow extends BaseClarkLogEntity<ScoreDividedIntoChargeLogFlow> {
    ...
    public ScoreDividedIntoChargeLogFlow(long pid, long unionId, long clubId, double curRemainder, double preValue, long exePid) {
        this.pid = pid;
        this.clubId = clubId;
    ...
    public String getInsertSql() {
        return "INSERT INTO ScoreDividedIntoChargeLog"
                + "(`serverId`,`timestamp`,`dateTime`,`pid`,`unionId`,`clubId`,`curRemainder`,`preValue`,`exePid`)"
                + "values(?, ?, ?, ?, ?, ?,?, ?, ?)";
    }

    public static String getCreateTableSQL() {
        String sql = "CREATE TABLE IF NOT EXISTS `ScoreDividedIntoChargeLog` ("
                ...
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='积分收益分成日志' DEFAULT CHARSET=utf8";
        return sql;
    }
